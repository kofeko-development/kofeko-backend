...
export const registerCompanyRequestSchema = z.object({
  body: z.object({
    ...
    companySize: z.enum(['1-10', '11-50', '51-200', '201-500', '501-1000', '1000+']).optional().default('1-10'),
    companyType: z.enum(['startup', 'enterprise', 'agency', 'non_profit']).optional().default('startup'),
    foundedYear: z.coerce.number().int().min(1800, 'Founded year is invalid').max(new Date().getFullYear(), 'Founded year cannot be in the future').optional().default(new Date().getFullYear()),
    companyWebsite: websiteUrlSchema.optional().default('https://example.com'),
    officialCompanyAddress: z.string().min(5, 'Official company address must be at least 5 characters').max(500).optional(),
    phoneNumber: z
      .string()
    ...
    companyLogo: optionalCompanyLogoSchema.optional().default(''),
    shortDescription: z.string().min(20, 'Short description must be at least 20 characters').max(1000).optional().default('Company profile details will be updated soon.'),
